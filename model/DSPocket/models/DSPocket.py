...
def load_pocket_repr(protein_path, proteins):
    all_protein_repr = []
    for protein in proteins:
        file_path = os.path.join(protein_path, '{}.npy'.format(protein))
        dict_load = np.load(file_path, allow_pickle=True)
        protein_tensor = torch.tensor(dict_load.tolist()['mol_repr_cls'])
        all_protein_repr.append(protein_tensor)

    combined_tensor = torch.stack(all_protein_repr, dim=0)
...
class UniMolModel(nn.Module):
    ...
    def __init__(self, output_dim=2, data_type='molecule', **params):
        ...
        self.protein_path = params.get('protein_path', None)
        if data_type == 'molecule':
        ...
        self.attention = MultiHeadAttentionNetwork(embed_dim=1024, num_heads=8, ff_dim=512)
    ...
    def forward(
        ...
        **kwargs
        ...
        all_repr = encoder_rep[:, :, :]  # all token repr  [B, S_L, 512]
        filtered_tensors = []
        filtered_coords = []
        ...
        protein_pocket_repr = load_pocket_repr(self.protein_path, protein) # [B, 512]
        
        pair_repr = torch.cat((protein_pocket_repr.to(cls_repr.device), cls_repr), axis=-1)

        pair_repr = self.attention(torch.unsqueeze(pair_repr, dim=1))
        logits = self.classification_head(pair_repr)
    ...
    def batch_collate_fn(self, samples):
        ...
        try:
            ...
            protein = [s[2] for s in samples]
        except:
            label = None
        return batch, label, protein
