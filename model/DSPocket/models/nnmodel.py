...
from .DSPocket import UniMolModel
...
class NNModel(object):
    ...
    def run(self):
        ...
        proteins = np.asarray(self.data['proteins'])
        ...
        for fold, (tr_idx, te_idx) in enumerate(self.splitter.split(X, y, group)):
            X_train, y_train, p_train = X[tr_idx], y[tr_idx], proteins[tr_idx]
            X_valid, y_valid, p_valid = X[te_idx], y[te_idx], proteins[te_idx]

            traindataset = NNDataset(X_train, y_train, p_train)
            validdataset = NNDataset(X_valid, y_valid, p_valid)
            if fold > 0:
    ...
    def evaluate(self, trainer=None,  checkpoints_path=None):
        ...
        testdataset = NNDataset(self.features, np.asarray(self.data['target']), self.data['proteins'])
...
def NNDataset(data, label=None, protein=None):
    """
    ...
    """
    return TorchDataset(data, label, protein)
...
class TorchDataset(Dataset):
    ...
    def __init__(self, data, label=None, protein=None):
        """
    ...
    def __getitem__(self, idx):
        ...
        r_data = self.data[idx]
        r_label = self.label[idx]
        r_protein = self.protein[idx]
        # print(r_label, r_protein)
        return r_data, r_label, r_protein
